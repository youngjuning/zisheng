...
const init = async () => {
  ...
  // 安装依赖
  execa.commandSync(
    `pnpm add commitizen -D eslint prettier @youngjuning/eslint-config-${eslintType} @youngjuning/prettier-config lint-staged yorkie`,
    {
      shell: true,
      stdout: 'inherit',
      stderr: 'inherit',
  ...
  if (eslintType.indexOf('typescript') !== -1) {
    execa.commandSync(`pnpm add commitizen -D typescript`, {
      shell: true,
      stdout: 'inherit',
      stderr: 'inherit',
    });
